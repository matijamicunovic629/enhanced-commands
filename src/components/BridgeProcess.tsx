...
export const BridgeProcess: React.FC<BridgeProcessProps> = ({ onClose }) => {
  ...
  const [usdcLogo, setUsdcLogo] = useState('');
  ...
  useEffect(() => {
    ...
    const fetchLogos = async () => {
      try {
        const usdcInfo = await getCoinInfo('usd-coin');
        const solInfo = await getCoinInfo('solana');
        
        if (usdcInfo && usdcInfo.image) {
          setUsdcLogo(usdcInfo.image);
        } else {
          setUsdcLogo('https://via.placeholder.com/40/4A90E2/FFFFFF?text=USDC');
        }
        
        if (solInfo && solInfo.image) {
          setSolLogo(solInfo.image);
        } else {
          setSolLogo('https://via.placeholder.com/40/4A90E2/FFFFFF?text=SOL');
        }
      } catch (error) {
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-4 mb-6">
        <img 
          ...
          className="w-12 h-12"
          onError={(e) => {
            (e.target as HTMLImageElement).src = "https://via.placeholder.com/48/4A90E2/FFFFFF?text=deBridge";
          }}
        />
        <div>
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center p-4 bg-white/5 rounded-lg">
            <div className="flex items-center gap-3">
              <img 
                src={usdcLogo || "https://via.placeholder.com/40/4A90E2/FFFFFF?text=USDC"}
                alt="USDC" 
                className="w-10 h-10"
                onError={(e) => {
            ...
            <div className="flex items-center gap-3">
              <img 
                src={solLogo || "https://via.placeholder.com/40/4A90E2/FFFFFF?text=SOL"}
                alt="SOL" 
                className="w-10 h-10"
                onError={(e) => {
                  (e.target as HTMLImageElement).src = "https://via.placeholder.com/40/4A90E2/FFFFFF?text=SOL";
                }}
              />
              <div>
        ...
        <>
          ...
          <div className="flex items-center gap-4 animate-pulse">
            ...
            <img 
              ...
              className="w-12 h-12"
              onError={(e) => {
                (e.target as HTMLImageElement).src = "https://via.placeholder.com/48/4A90E2/FFFFFF?text=SOL";
              }}
            />
          </div>
